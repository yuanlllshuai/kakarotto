...
const Index: FC<IndexProps> = ({ data }) => {
      ...
      return (
        <div key={i.key}>
          {renderItem(
            i,
            l,
            routes.includes(parentKey)
              ? styles.item_node_active
              : styles.item_node
          )}
          {i.children && (
    ...
    const content = (
      <>
        <div className={styles.item_left}>
          ...
          <span className={haveChildren || isLevel0 ? styles.title : ""}>
            {label}
          </span>
        </div>
        {haveChildren &&
          (routes.includes(key) ? (
            <Icon name="shangup" size="16" />
          ) : (
            <Icon name="xiadown" size="16" />
          ))}
      </>
    );
    // 一级节点没有子节点
  ...
  return <div className={styles.main}>{renderTree(data, -1, "")}</div>;
};
